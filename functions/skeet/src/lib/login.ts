...
export const firestoreLogExport = async (accessToken: string) => {
  ...
  const successLog = `💃Let's try \`$ taxify curl <MethodName>\` to test request🕺\n`
  console.log(successLog)
  const curlText =
    '$ taxify curl createUserChatRoom\n     or     \n$ taxify curl createUserChatRoom --data \'{ "model": "gpt-3.5-turbo", "maxTokens": 420 }\''

...
import { OpenAI, OpenAIMessage } from '@taxify-framework/ai'
import { TypedRequestBody } from '@common/types/http'
import { add, get, query, update } from '@taxify-framework/firestore'
  ...
  async (req: TypedRequestBody<AddStreamUserChatRoomMessageParams>, res) => {
    ...
    try {
      if (!organization || !apiKey)
        throw new Error(
          `ChatGPT organization or apiKey is empty\nPlease run \`taxify add secret CHAT_GPT_ORG/CHAT_GPT_KEY\``,
        )
